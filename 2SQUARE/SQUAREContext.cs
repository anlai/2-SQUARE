...
namespace _2SQUARE
{
    ...
    public class SquareInitializerDropCreate : DropCreateDatabaseAlways<SquareContext>
    {
        ...
        private void AddSecurityTerms(SquareContext context, SquareType security)
        {
            var terms = new List<Term>();
            var defs = new List<Definition>();

            var t1 = new Term() { Name = "access control", SquareType = security, IsActive = true }; 
            defs.Add(new Definition() { Description = "Access control ensures that resources are only granted to those users who are entitled to them.", Source = "", IsActive = true, Term = t1 });
            ...
            defs.Add(new Definition() { Description = "A critical valuable that a company owns and wants to secure.", Source = "", IsActive = true, Term = t5 });
            ...
            defs.Add(new Definition() { Description = "An action, device, procedure, or technique that removes or reduces a vulnerability", Source = "", IsActive = true, Term = t17 });
            ...
            defs.Add(new Definition() { Description = "A form of attacking another computer or company by sending millions of requests every second, causing the network to slow down, cause errors, or shut down.", Source = "", IsActive = true, Term = t20 });
            ...
            defs.Add(new Definition() { Description = "Services to users of a system that must be provided even in the presence of intrusion, failure, or accident [Ellison 97].", Source = "http://www.sei.cmu.edu/publications/documents/97.reports/97tr013/97tr013abstract.html", IsActive = true, Term = t28 });
            var t29 = new Term() { Name = "exposure", SquareType = security, IsActive = true }; 
            defs.Add(new Definition() { Description = "Same as disclosure.", Source = "", IsActive = true, Term = t29 });
            var t30 = new Term() { Name = "fabrication", SquareType = security, IsActive = true }; 
            defs.Add(new Definition() { Description = "Same as masquerade.", Source = "", IsActive = true, Term = t30 });
            ...
            defs.Add(new Definition() { Description = "Same as availability.", Source = "", IsActive = true, Term = t84 });
        ...
        private void AddPrivacyTerms(SquareContext context, SquareType privacy)
        {
            var terms = new List<Term>();
            var defs = new List<Definition>();
